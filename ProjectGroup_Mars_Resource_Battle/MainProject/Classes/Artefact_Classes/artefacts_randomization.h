...
//// ќ—Ќќ¬Ќјя ‘”Ќ ÷»я  ќ“ќ–”ё Ќ”∆Ќќ «јё«ј“№ ƒЋя ѕќЋ”„≈Ќ»я ¬≈ “ќ–ј ј–“≈‘ј “ќ¬
// cnt - количество артефактов. size - размер пол€
vector <artefact*> get_list_of_artefacts(int cnt, int size) {
    ...
    // вектор дл€ рандомизации видов артефактов 1, 2, 3
    vector <int> Random_pos (cnt, 0);

    ////////////////////// јналогично как сверху
    // 1 = "TCube", 2 = "TSphere", 3 = "TCylinder"
    vector <int> Form = {1, 2, 3};
    vector <int> Random_form (cnt, 0);
    ///////////////////////////////////////////

    // заполн€ю массив только ширпотребом и обычными
    for (int i = 0; i < cnt; ++i) {
        Random_pos[i] = rand() % 2 + 1;
    }
    // добавл€ю редкие в рандомные позиции
    for (int i = 0; i < cnt / 10 + 1; ++i) {
        Random_pos[rand() % cnt - 1] = 3;
    }

    ////////////////////// добавл€ю рандомные чиселки дл€ формы
    for (int i = 0; i < cnt; ++i) {
        Random_form[i] = rand() % 3 + 1;
    }
    /////////////////////////////

    // вектор став€щий каждому артефакту пару координат х и у
    ...
    for (int i = 1; i < cnt; ++i) {
        // флаг будет провер€ть есть ли уже така€ пара координат в ранее инициализированных артефактах
        ...
        if (flag) {
            // если флаг, то ставим эту пару координат в зависимости от вида артефакта мен€ет его координаты
