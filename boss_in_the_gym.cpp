...
void frequent::print() {
    cout << "¥¤ª®áâì : "<< _availability << "\n";
    cout << "Š®®à¤¨­ âë : " << _cord_x << " ; " << _cord_y << "\n";
}
...
public:
    ...
    void generateAllHills() {
        ...
        while(count_hills > 0)
        {
            ...
            // £¥­¥à¨àã¥¬ á«ãç ©­ãî è¨à¨­ã £®àë
            int width_hill = (rand() * rand() % width);
            // £¥­¥à¨àã¥¬ á«ãç ©­ãî ¢ëá®âã £®àë (¯® ã)
...
class Robot {
private:
    int x, y, z; // ª®®à¤¨­ âë à®¡®â 
    int rotation; // â¥ªãé¥¥ ­ ¯à ¢«¥­¨¥ à®¡®â  (0-3 á®®â¢¥âáâ¢¥­­® "¢¯¥à¥¤", "­ ¯à ¢®", "­ § ¤", "­ «¥¢®")
    bool hasArtifact; // ¥áâì «¨ ã à®¡®â   àâ¥ä ªâ (¤ /­¥â)
    int score; // ª®«¨ç¥áâ¢® ®çª®¢ ¨£à®ª 
    int numArtifacts;  // ª®«¨ç¥áâ¢®  àâ¥ä ªâ®¢ ­  à®¡®â¥
    int health; // §¤®à®¢ì¥ à®¡®â 
    int load_capacity; // £àã§®¯®¤ê¥¬­®áâì à®¡®â , ¯® ¤¥ä®«âã 100
